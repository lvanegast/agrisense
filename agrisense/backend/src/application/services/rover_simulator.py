...
class RoverSimulatorService:
    ...
    def __new__(cls, *args, **kwargs):
        if not cls._instance:
            cls._instance = super(RoverSimulatorService, cls).__new__(
                cls, *args, **kwargs
            )
        ...
        async with self._lock:
            ...
            if self.status == "charging":
                ...
                if self.battery >= 100.0:
                    ...
                    self.status = "patrolling"
                    self.logs.append(
                        f"[{timestamp}] Battery fully charged. Resuming patrol loop."
                    )
                return
            ...
            if self.battery <= 15.0:
                ...
                self.x, self.y = (
                    25.0,
                    25.0,
                )  # Volver a la estación de carga rápida (cerca de la base)
                self.logs.append(
                    f"[{timestamp}] Low battery warning ({self.battery:.1f}%). Returning to Charging Station."
                )
                return
            ...
            if self.status == "sampling":
                self.status = "patrolling"
                self.logs.append(
                    f"[{timestamp}] Multispectral scan completed. Navigating to next target."
                )
                return
            ...
            if distance < 2.5:
                ...
                self.logs.append(
                    f"[{timestamp}] Waypoint {self.current_waypoint_index + 1} reached. Activating LiDAR & NDVI sensor..."
                )
                return

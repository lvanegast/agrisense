...
class ZoneModel(Base):
    ...
    sensors = relationship("SensorModel", back_populates="zone", lazy="selectin", cascade="all, delete-orphan")
    alerts = relationship("AlertModel", back_populates="zone", lazy="selectin", cascade="all, delete-orphan")
    actuators = relationship("ActuatorModel", back_populates="zone", lazy="selectin", cascade="all, delete-orphan")
    work_orders = relationship("WorkOrderModel", back_populates="zone", lazy="selectin", cascade="all, delete-orphan")
...
class SensorModel(Base):
    ...
    readings = relationship("ReadingModel", back_populates="sensor", lazy="selectin", cascade="all, delete-orphan")
...
class ReadingModel(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    sensor_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("sensors.id", ondelete="CASCADE"), nullable=False)
...
class RuleModel(Base):
    ...
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    action_type: Mapped[str] = mapped_column(String(50), default="alert")  # alert | work_order | both
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class WorkOrderModel(Base):
    ...
    zone_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("zones.id", ondelete="CASCADE"), nullable=False)
    sensor_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("sensors.id", ondelete="SET NULL"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
